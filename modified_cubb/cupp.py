...
def kombone(seq, start, special=""):
    for mystr in seq:
        for mystr1 in start:
            yield mystr + special + mystr1


def kombtwo(seq, start, special_chars="!@#$%^&*"):
    for base_str in seq:
        for suffix in start:
            # Basic combination without any special characters in the middle
            yield base_str + suffix

            # Insert special characters only at the beginning and end to reduce output size
            for char in special_chars:
                # Add special character at the beginning of the base string
                yield char + base_str + suffix

                # Add special character at the end of the base string
                yield base_str + suffix + char



def kombthree(seq, start, special_chars="!@#$%^&*"):
    for base_str in seq:
        for suffix in start:
            # Basic combinations without any special characters in the middle
            yield base_str + suffix
            
            # Insert special characters only in specific positions to control output size
            for char in special_chars:
                # Add special character at the beginning of the base string
                yield char + base_str + suffix
                
                # Add special character in the middle of the base string (if length allows)
                if len(base_str) > 1:
                    middle_index = len(base_str) // 2
                    new_str = base_str[:middle_index] + char + base_str[middle_index:]
                    yield new_str + suffix

                # Add special character at the end of the base string (alternative to `base_str + suffix`)
                yield base_str + char + suffix


def kombfour(seq, start, special_chars="!@#$%^&*"):
    for base_str in seq:
        for suffix in start:
            # Yield with special characters at the end (original)
            yield base_str + suffix
            # Yield with special characters at different positions
            for i in range(1, len(base_str)):
                for char in special_chars:
                    # Insert special character at each position in the base_str
                    new_str = base_str[:i] + char + base_str[i:]
                    yield new_str + suffix


def kombfive(seq, suffixes, special_chars="!@#$%^&*:"):
    for base_str in seq:
        for suffix in suffixes:
            # Original combination without special characters in the middle
            yield base_str + suffix

            # Combinations with special characters inserted in various positions
            for i in range(1, len(base_str)):
                for char in special_chars:
                    # Insert special character at position `i` within `base_str`
                    new_str = base_str[:i] + char + base_str[i:]
                    yield new_str + suffix
                    
                    # Add special character between `base_str` and suffix
                    yield base_str + char + suffix

                    # Add special characters in both middle and between base and suffix
                    for j in range(1, len(new_str)):
                        if j != i:  # Avoid duplicating the first character insertion
                            combined_str = new_str[:j] + char + new_str[j:]
                            yield combined_str + suffix
...
def interactive():
    ...
    complexity_level = input("> Complexity level (1-5): ").lower()
    while len(complexity_level) == 0 or complexity_level == " " or complexity_level == "  " or complexity_level == "   ":
        print("\r\n[-] You must enter a some level !")
        complexity_level = input("> Complexity level (1-5): ").lower()
    profile["complexity_level"] = int(complexity_level)
...
def generate_wordlist_from_profile(profile):
    ...
    if profile["complexity_level"] == 1:
        komb = kombone
    elif profile["complexity_level"] == 2:
        komb = kombtwo
    elif profile["complexity_level"] == 3:
        komb = kombthree
    elif profile["complexity_level"] == 4:
        komb = kombfour
    elif profile["complexity_level"] == 5:
        komb = kombfive
    else:
        komb = kombone  # Optional: for values outside the expected range
